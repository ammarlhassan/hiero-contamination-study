...
% Uncomment for final camera-ready version
% \aclfinalcopy
...
\textbf{Script Reproduction}: We retrained M2M-100 using the original \texttt{train.py} with its default hyperparameters (epochs=20, batch\_size=16, lr=3e-5, Adam, no warmup, no label smoothing). This represents the closest possible replication of the original training procedure.
...
Exact matching finds 15/50 (30.0\%) byte-identical targets; after normalization, \textbf{16/50 (32.0\%)} match---one additional near-duplicate differing only in bracket notation. The validation set shows a comparable rate (23/75, 30.7\%). At the 70\% threshold with character 8-grams, 25/50 (50\%) of test samples are flagged, confirming that 32\% exact match is a conservative lower bound. The consistent rates across test and validation sets indicate a systematic characteristic of the splitting procedure.
...
\midrule
Released & All & 50 & 37.0 & 56.2 \\
         & Contaminated & 16 & 60.3 & 81.5 \\
         & Clean & 34 & 30.9 & 48.1 \\
\addlinespace
Script Reproduction & All & 50 & 42.2 & 61.2 \\
               & Contaminated & 16 & 77.5 & 85.1 \\
               & Clean & 34 & 33.8 & 53.7 \\
\addlinespace
M2M-100 Hybrid & All & 50 & 46.8 & 63.3 \\
               & Contaminated & 16 & \textbf{83.8} & \textbf{91.9} \\
               & Clean & 34 & 37.3 & 54.0 \\
\addlinespace
M2M-100 Conservative & All & 50 & \textbf{47.3} & \textbf{63.2} \\
                & Contaminated & 16 & 77.9 & 90.6 \\
                & Clean & 34 & \textbf{39.2} & \textbf{54.2} \\
\addlinespace
mBART-50 & All & 50 & 41.9 & 59.6 \\
         & Contaminated & 16 & 72.8 & 87.3 \\
         & Clean & 34 & 33.7 & 50.6 \\
\bottomrule
\end{tabular}
\caption{Translation quality by contamination status across five models (\texttt{case:lc} BLEU). All models show substantial contamination gaps (29--47 BLEU points). Bold indicates best performance per subset.}
\label{tab:main-results}
\end{table*}

The gap is substantial across all five models (+29 to +47 BLEU), holding across architectures (M2M-100 vs.\ mBART-50) and training strategies---the problem is inherent to the dataset. The Script Reproduction outperforms the Released Model (+5.2 BLEU) despite identical hyperparameters; the gap is predominantly on contaminated samples (+17.2) versus clean (+2.9), consistent with different checkpoint selection or training seed rather than a systematic quality difference.

\paragraph{Confidence Intervals.} Bootstrap 95\% CIs (1,000 iterations) are wide due to small subsets (e.g., Released clean: 30.9 [16.2--43.6]) but the gap direction is consistent across all models and all bootstrap samples.
...
\paragraph{Retrieval Baseline.} An oracle retrieval baseline that copies the best-matching training target (by character 8-gram coverage against the \emph{test target}) achieves 81.8 BLEU on exact-match items---rivaling the best neural model (77.9)---while scoring 0.0 on clean items. A non-oracle source-KNN variant that uses only source similarity achieves only 22.0 BLEU on exact-match items. The 59.8-point gap directly measures the contribution of target memorization over source-driven translation.

\paragraph{Three-Tier Stratification.} Partitioning the test set into exact match (n=16), soft leakage (70--99\% 8-gram overlap; n=9), and clean ($<$70\%; n=25)\footnote{This three-tier ``clean'' subset (n=25) is stricter than the binary split used in Table~\ref{tab:main-results}, where ``clean'' (n=34) includes the 9 soft-leakage samples.} shows BLEU decreasing monotonically across tiers for all five models, confirming that performance tracks contamination severity.
