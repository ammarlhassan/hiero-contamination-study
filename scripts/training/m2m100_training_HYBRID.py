#!/usr/bin/env python3
"""
M2M-100 Training with HYBRID Strategy - AGGRESSIVE VERSION
Combines hieroglyphics paper's learning rate with WMT 2025 optimizations

Key Hybrid Strategy:
1. Learning Rate: 3e-5 (from hieroglyphics paper - more aggressive)
2. Warmup: 1000 steps + Cosine scheduler (WMT 2025)
3. 5x upsampling for low-resource pairs (WMT 2025)
4. Label smoothing: 0.15, Dropout optimization (WMT 2025)
5. Mixed precision training + Early stopping (WMT 2025)

Base Model: facebook/m2m100_418M (same as hieroglyphics paper)
Strategy: Aggressive LR from hieroglyphics paper + WMT 2025 optimizations
"""
...
print("="*80)
print("M2M-100 TRAINING WITH HYBRID STRATEGY - AGGRESSIVE VERSION")
print("="*80)
print()
print("Hybrid Strategy:")
print("  1. LR: 3e-5 (from hieroglyphics paper - more aggressive)")
print("  2. Warmup + Cosine scheduler (WMT 2025)")
print("  3. 5x upsampling for low-resource pairs (WMT 2025)")
print("  4. Strategic dropout and weight decay (WMT 2025)")
print("  5. Early stopping with patience (WMT 2025)")
print("="*80)
print()
...
# Base: Hieroglyphics paper's LR (3e-5) + WMT 2025 optimizations
EPOCHS = 20
...
LEARNING_RATE = 3e-5  # HIEROGLYPHICS PAPER (more aggressive)
WARMUP_STEPS = 1000   # WMT 2025 optimal for ~60K data
WEIGHT_DECAY = 0.1    # WMT 2025 strong regularization
LABEL_SMOOTHING = 0.15  # WMT 2025 reduce overfitting
MAX_LENGTH = 112
...
OUTPUT_DIR = "m2m100_checkpoints_HYBRID"
...
print(f"Learning Rate: {LEARNING_RATE} (HIEROGLYPHICS PAPER - AGGRESSIVE)")
print(f"Warmup Steps: {WARMUP_STEPS} (WMT 2025 optimal for ~60K)")
print(f"Scheduler: Cosine with warmup (WMT 2025 proven superior)")
print(f"Weight Decay: {WEIGHT_DECAY} (WMT 2025)")
print(f"Label Smoothing: {LABEL_SMOOTHING} (WMT 2025)")
print(f"Dropout: {DROPOUT} / {ATTENTION_DROPOUT} / {ACTIVATION_DROPOUT} (WMT 2025)")
...
final_state = {
    'model': MODEL_NAME,
    'strategy': 'Hybrid (Hieroglyphics LR + WMT 2025)',
    ...
    'hybrid_config': {
        ...
        'learning_rate_source': 'Hieroglyphics paper (aggressive)',
